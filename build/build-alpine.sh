...
if [ "$#" -eq 1 ]; then
  ...
  if [ "$ALPINE_VERSION" == "latest" ]; then
    declare -r TAG_VERSION="alpine-edge"
  else
  ...
  if [ "$NOPUBLISH" ]; then
    ...
    docker build --file build/alpine/Dockerfile --compress --force-rm --pull --tag "${TAG_NAME}:${TAG_VERSION}" --build-arg "ALPINE_VERSION=${ALPINE_VERSION}" .
  elif [ "$PUSHONLY" ]; then
...
for ALPINE_VERSION in "${ALPINE_VERSION_TARGETS[@]}"; do
  if [ "$ALPINE_VERSION" == "latest" ]; then
    declare TAG_VERSION="alpine-edge"
  else
